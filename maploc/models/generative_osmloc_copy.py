...
Copy of generative_osmloc.py from before the changes made in this chat.
"""
...
class PriorNetwork(nn.Module):
    """Image features (pooled) → μ_prior, log_σ_prior."""
    ...
    def forward(self, f_img):
        """f_img: (B, C, H, W) → mu (B, D), log_sigma (B, D)."""
        h = self.net(f_img)
...
class PosteriorEncoder(nn.Module):
    """True MIA semantic mask → μ_post, log_σ_post.  Training only."""
...
class ConditionalDecoder(nn.Module):
    """Latent Z + DINOv2 image features → BEV features + 9-class semantic map.

    The decoder upsamples the latent code spatially, fuses it with image
    features, and produces BEV representations at the target spatial size.
    """
    ...
    def __init__(self, latent_dim, img_dim, matching_dim, sem_channels,
        ...
        self.z_init_h = bev_h // 8
        self.z_init_w = bev_w // 8
    ...
    def forward(self, z, f_img):
        ...
        z_spatial = F.interpolate(
            z_spatial, (self.bev_h, self.bev_w), mode="bilinear", align_corners=False
        )

        f_img_resized = F.interpolate(
            f_img, (self.bev_h, self.bev_w), mode="bilinear", align_corners=False
        )
        fused = self.fuse(torch.cat([z_spatial, f_img_resized], dim=1))

        features = self.blocks(fused)

        f_bev = self.matching_head(features)
        sem_bev = self.semantic_head(features)
        f_bev_sem = self.sem_align(features)

        return f_bev, sem_bev, f_bev_sem, features


class MapSemanticHead(nn.Module):
    """Takes MapEncoder raw embeddings → 9-class semantic prediction."""
...
class generative_osmloc_copy(BaseModel):
    default_conf = {
        "image_encoder": "???",
        "map_encoder": "???",
        "latent_dim": 256,
    ...
    def _forward(self, data):
        ...
        sem_map = self.map_sem_head(embedding_map)
        ...
        if self.training and "semantic_mask" in data:
            sem_gt = data["semantic_mask"]
            uv_gt = data["uv"]
            bev_h, bev_w = self.decoder.bev_h, self.decoder.bev_w
            B_sz = sem_gt.shape[0]
            crops = []
            for b in range(B_sz):
                r_c = int(uv_gt[b, 1].round())
                c_c = int(uv_gt[b, 0].round())
                r0 = r_c - bev_h // 2
                c0 = c_c - bev_w // 2
                r0 = max(0, min(r0, sem_gt.shape[2] - bev_h))
                c0 = max(0, min(c0, sem_gt.shape[3] - bev_w))
                crops.append(sem_gt[b, :, r0:r0 + bev_h, c0:c0 + bev_w])
            sem_gt_post = torch.stack(crops, dim=0)
            mu_post, log_sigma_post = self.posterior_net(sem_gt_post)
        ...
        scores, templates, valid_templates = self.exhaustive_voting(f_bev, f_map, valid_bev)
    ...
    def loss(self, pred, data):
        ...
        if "semantic_mask" in data:
            sem_gt = data["semantic_mask"]
            sem_bev = pred["sem_bev"]
            bev_h, bev_w = sem_bev.shape[2], sem_bev.shape[3]

            crops = []
            for b in range(sem_gt.shape[0]):
                r_c = int(xy_gt[b, 1].round())
                c_c = int(xy_gt[b, 0].round())
                r0 = max(0, min(r_c - bev_h // 2, sem_gt.shape[2] - bev_h))
                c0 = max(0, min(c_c - bev_w // 2, sem_gt.shape[3] - bev_w))
                crops.append(sem_gt[b, :, r0:r0 + bev_h, c0:c0 + bev_w])
            sem_gt_crop = torch.stack(crops, dim=0)

            loss_sem_bev = F.binary_cross_entropy_with_logits(
                sem_bev, sem_gt_crop, reduction="none"
            ).mean(dim=(1, 2, 3))
            loss_dict["loss_sem_bev"] = loss_sem_bev
            total = total + self.conf.lambda_sem_bev * loss_sem_bev

            sem_map = pred["sem_map"]
