...
class MapLocDataset(torchdata.Dataset):
    ...
    def get_view(self, idx, scene, seq, name, seed, bbox_tile):
        ...
        depth = None
        if self.depth_dirs is not None:
            depth = read_image(self.depth_dirs[scene] / (name + self.depth_ext))
        ...
        if flip is not None:
            roll = -roll if flip is True else roll # negative sign for mirrored flip

        if depth is not None:
            depth, _, _, _, _ = self.process_image(
                depth, cam, roll, pitch, seed,jitter = False
            ) # rectify + bilinear resample + zero padding

        image, valid, cam, roll, pitch = self.process_image(
            image, cam, roll, pitch, seed,jitter = True
        ) # rectify + bilinear resample + zero padding + color jitter
    ...
    def process_image(self, image, cam, roll, pitch, seed, jitter = True):
        image = (
