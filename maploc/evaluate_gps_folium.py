...
def plot_filtered_gps(csv_path, output_html, sample_rate=5, error_threshold=30.0):
    ...
    # Calculate actual error bounds directly from metric X/Y
    ...
    print(f"Dropped {outliers_dropped} prediction outliers (> {error_threshold}m error) out of {len(df)}")

    # 2. Downsample
    # User requested: "sample like 2 from every 10hz" -> 2/10 = 1/5 of the data
    df_sampled = df_filtered.iloc[::sample_rate].copy()
    print(f"Sampling 1 in {sample_rate} valid frames. Total points to plot: {len(df_sampled)}")
    ...
    # Plot valid predictions as markers with trace lines to GT anchor
    for _, row in df_sampled.iterrows():
        ...
        CircleMarker(
            ...
            fill_color="red",
            fill_opacity=1.0,
            tooltip=f"{row['name']} | err: {row['error_m']:.2f}m"
        ).add_to(pred_group)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python3 evaluate_gps_folium.py <input.csv> <output.html>")
        sys.exit(1)
